...
async function loadThreadMessages(threadId) {
    try {
        ...
        // Add system message
        addMessage('system', `Switched to thread: ${data.thread.title}`);

        // Load tasks and their results
        if (data.tasks && data.tasks.length > 0) {
            data.tasks.forEach(task => {
                // Add user message
                addMessage('user', task.prompt, task.id);

                // Add assistant response if completed
                if (task.status === 'completed' && task.result) {
                    addMessage('assistant', task.result.response, task.id, task);
                } else if (task.status === 'failed') {
                    addMessage('error', `Task failed: ${task.error}`, task.id);
                } else if (task.status === 'running') {
                    addMessage('system', `Task is running... (Worker: ${task.worker_id})`, task.id);
                } else if (task.status === 'queued') {
                    addMessage('system', `Task is queued...`, task.id);
                }
            });
        }
...
async function sendMessage() {
    ...
    try {
        ...
        addMessage('system', `Task ${data.task_id} queued. Status: ${data.status}`);
        ...
        addMessage('error', `Failed to submit task: ${error.message}`);
    }
}
    ...
    const poll = async () => {
        try {
            ...
            if (task.status === 'completed') {
                // Task completed, add response
                addMessage('assistant', task.result.response, taskId, task);
                return;
            } else if (task.status === 'failed') {
                // Task failed
                addMessage('error', `Task failed: ${task.error}`, taskId);
                return;
            } else if (task.status === 'running' && attempts % 5 === 0) {
                // Update status every 5 attempts
                updateMessage(taskId, `Task is running... (Worker: ${task.worker_id})`);
            }
            ...
            if (attempts < maxAttempts) {
                ...
                addMessage('error', `Task ${taskId} timed out`);
            }
...
function updateMessage(taskId, content) {
    const messages = document.querySelectorAll('.message');
    messages.forEach(msg => {
        if (msg.dataset.taskId === taskId) {
...
// Modified addMessage to include task ID
function addMessage(role, content, taskId = null, taskData = null) {
    const chatMessages = document.getElementById('chat-messages');
